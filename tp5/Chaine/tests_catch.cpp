...
TEST_CASE("Test afficherParValeur() et afficherParReference()") {
    Chaine::nbConstructeurParCopie = 0;
    
    const char * original = "une chaine a tester";
    const Chaine c1(original);
    const Chaine c2(original);

    // Appel de la fonction afficherParReference() avec une référence sur une instance de Chaine en paramètre
    afficherParReference(c1);

    // Vérification que le constructeur de copie n'a pas été appelé
    REQUIRE( Chaine::nbConstructeurParCopie == 0 );

    // Appel de la fonction afficherParValeur() avec une instance de Chaine en paramètre
    afficherParValeur(c1);

    // Vérification que le constructeur de copie a été appelé
    REQUIRE( Chaine::nbConstructeurParCopie == 1 );
}
